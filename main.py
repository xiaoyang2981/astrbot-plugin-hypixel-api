...
class HypixelPlugin(Star):
    ...
    async def _handle_blitz(self, event, name):
        if not self.client:
            yield event.plain_result("Hypixel API Key 未配置"); return
        try:
            b = await self.client.get_blitz_stats(name=name)
        except Exception as e:
            logger.info(f"布吉岛 {name} 失败: {e}")
            yield event.plain_result(f"获取布吉岛数据失败: {e}"); return
        kdr = calc_kdr(b["kills"], b["deaths"])
        yield event.plain_result(
            f"━━━━ ⚔️ 布吉岛 ━━━━\n"
